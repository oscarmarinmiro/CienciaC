...
ccviz.viz.series_users = function(options)
{
    ...
    self.TIME_SERIES_HEIGHT_FACTOR = 0.8;
    self.LEFT_MARGIN = 30;
...
//        console.log(data);
        ...
        var y_scale = d3.scale.linear().domain([d3.min(data, function(d,i){return d.price}),d3.max(data, function(d,i){return d.price})]).range([(self.height*self.TIME_SERIES_HEIGHT_FACTOR)-self.TITLE_OFFSET,self.TITLE_OFFSET]).clamp(true);
...
//        console.log(my_data);
        ...
        join.enter().append("rect")
            .attr("x", function(d,i){return hor_scale(d.column-0.5)})
